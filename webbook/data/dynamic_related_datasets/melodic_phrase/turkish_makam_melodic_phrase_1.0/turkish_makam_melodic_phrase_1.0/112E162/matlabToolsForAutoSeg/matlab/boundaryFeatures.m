function boundaryFeatures(symbTrFile,makamHist,usulHist,usulFile,midiNo)
...
[NM, bolut] = symbtr2nmat(symbTrFile,usulFile);
if(length(NM)<7)
    disp('NMde cok az veri var:');disp(symbTrFile);return;
else
    [NM]=filterNoteMatrix(NM);
end
if(length(bolut)>2)%eger bolut iceren bir dosya ise
    %check Nan bolut
    NanVar=0;
    for k=1:length(bolut)
        if(isnan(bolut(k).beat))
...
for k=1:length(NM)-1
    beatStart=NM(k,1);
    boundBeatProbOnset(k)=segInUsulHisto(1+mod(round(beatStart*2),zaman*2));
    beatNextStart=NM(k+1,1);maxVal=0;
    for b=beatStart+0.5:0.5:beatNextStart
        bmod= 1 + mod(round(b*2), zaman*2);
...
if (length(bolut)<2)%Bolutlenmemis dosya, son satira -1 yaz
    ...
    for k=1:length(bolut)%uzman bolut bilgisinin son sutuna islenmesi
        if(~isempty(findstr('53',char(bolut(k).kod))))
            ind=find(featureMatrix(:,1)==bolut(k).beat);
            if(isempty(ind) && k~=length(bolut))%bolut beat'e karsilik gelmiyor ise sonrakine esle 
                ind=find((featureMatrix(:,1)-bolut(k).beat)>0);
                if(~isempty(ind))
    ...
    %Elle segmentasyonu ayri bir dosyaya yazdir (autoSeg ile gorsel karsilastirma amaclidir)
    manSegFileName=strrep(symbTrFile,'.txt','.manSeg');
    fidOut2=fopen(manSegFileName,'w+t');
    fprintf(fidOut2,'Boundary(first note onset) in beats:\r\n');
    for k=1:length(bolut)%uzman bolut bilgisinin son sutuna islenmesi
        if(~isempty(findstr('53',char(bolut(k).kod))))
            fprintf(fidOut2,'%4.4f\r\n',bolut(k).beat);
        end
    end
    fclose(fidOut2);
...
%------------------------------------------
%OZNITELIKLERIN DOSYAYA YAZILMASI / WRITING FEATURES TO FILE 

outFile=strrep(symbTrFile,'.txt','.ptxt');
fid=fopen(outFile,'w+t');
for k=1:length(featureMatrix)
    for m=1:size(featureMatrix,2)
        if(~isnan(sum(featureMatrix(k,:))))
            fprintf(fid,'%4.4f\t',featureMatrix(k,m));
        else
            disp('veri NaN iceriyor, dosyaya yazilmadi');
        end
    end
    fprintf(fid,'\n');
end
