function [b] = boundary_tr(nmat, fig, bolut)
...
if fig==1
	pianoroll(nmat); 
    %set(gca,'Position',[0.05 0.55 0.9 0.4]); xl=get(gca,'XLim');
	%subplot('Position',[0.05 0.05 0.9 0.4])
    a=axis;
    axis([a(1) a(2) 55 85]);hold;
    for k=1:length(b)
        line([on(k) on(k)],[55 55+b(k)*10],'Color','b','LineWidth',2);
    end
    for k=1:length(bolut)
        if(~isempty(findstr('53',char(bolut(k).kod))))%Baris: sadece ezgi sýnýrlarýný çizmek için
            line([bolut(k).beat bolut(k).beat],[62 65],'Color','g','LineWidth',3);
        end
    end
